<template>
  <v-navigation-drawer class="fill-height" elevation="10" :location="smAndDown ? 'right' : 'left'" eager permanent
    :expand-on-hover="mode !== 'mini'" :rail="mode !== 'full'" @click="if (!smAndDown) mode = modes.next;"
    @update:rail="(value) => (sidebarExpanded = !value)">
    <slot name="logo" :sidebarExpanded="sidebarExpanded"> </slot>
    ...
    <template #append>
      <div class="d-flex flex-column mx-auto">
        ...
        <v-btn size="large" variant="text" icon="mdi-github" target="_blank"
          href="https://github.com/SelfhostedPro/Yacht" @click.stop />
      </div>
    </template>
  </v-navigation-drawer>
</template>
