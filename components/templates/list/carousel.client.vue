<template>
  <v-card v-bind="$attrs" height="50vh" class="text-center d-flex align-center" rounded="0" variant="flat">
    <v-slide-group @mouseenter="hovered = true" @mouseleave="hovered = false" mandatory :show-arrows="false"
      v-model="carousel" center-active>
      <v-slide-group-item v-for="featuredApp in template.featured" :key="template.templates[featuredApp].title"
        v-slot="{ isSelected, toggle, selectedClass }">
        <v-img cover class="d-flex align-end featured-image mx-3 my-3 rounded" @click="toggle"
          :color="isSelected ? 'primary' : 'foreground'" max-width="50vw" min-width="30vw" height="50vh"
          :aspect-ratio="isSelected ? 16 / 9 : 4 / 3"
          :src="template.templates[featuredApp].featured_image || template.templates[featuredApp].logo">
          <v-card rounded="0" class="featured-card">
            ...
            <v-card-text v-if="isSelected && template.templates[featuredApp].description" style="height: 60px;"
...
const startTimeout = () => {
  autoplayTimeout.value = window.setTimeout(() => {
    template.featured?.length && template.featured.length - 1 > carousel.value ? carousel.value++ : carousel.value = 0
    resetTimeout()
  }, 10000)
}
const resetTimeout = () => {
...
.v-slide-group__prev {
  display: none !important;
    ...
    </v-carousel> -->~/shared/templates/yacht