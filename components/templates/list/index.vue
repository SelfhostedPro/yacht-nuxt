  ...
  <v-container fluid class="px-0">
    ...
    <v-toolbar class="px-2">
      <v-row justify="space-between">
        <v-col>
          <v-text-field v-model="search" clearable density="comfortable" @click:clear="search = ''" hide-details
            placeholder="Search" prepend-inner-icon="mdi-magnify" style="max-width: 300px;" variant="solo" />
        </v-col>
    ...
    <v-dialog v-model="openInfo" :max-width="maximize ? undefined : '800'" :fullscreen="maximize"
      ...
      <template v-slot:default>
        <v-card color="background" class="overflow-auto">
          <common-title-bar :title="`${selectedApp?.title || selectedApp?.name} info`" color="primary"
            @maximize="maximize = !maximize" :closable="true" @close="openInfo = false; selectedApp = undefined" />
          <v-card-text>
