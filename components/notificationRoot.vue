<template>
  <Notivue v-slot="item">
    <NotivueSwipe :item="item">
      <Notification :item="(item as NotivueItem<Notification>)" />
    </NotivueSwipe>
  </Notivue>
</template>

<script lang="ts" setup>
import { Notivue, NotivueSwipe, type NotivueItem } from 'notivue'
    ...
    const sse = useSse('/api/notifications', {
      async onopen(response) {
        if (response.ok) {
          ...
          useToast({ title: 'Error', level: 'error', message: `Failed to connect to notifications SSE: ${response.statusText}`, dedupe: true })
