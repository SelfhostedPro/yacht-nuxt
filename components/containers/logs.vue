<template>
  <Xterm ref="xterm" v-if="attachAddon" :attach-addon="attachAddon" term-type="logs" @close="$emit('close')"
    @refresh="refresh()">
    <template #btns>
...
import { AttachAddon } from '~/composables/containers/xterm-ws-to-sse';
...
const getLogs = async () => {
  ...
  const { eventSource, error, close } = useEventSource(`/api/containers/${server}/${name}/logs`)
  ...
  attachAddon.value = new AttachAddon(eventSource.value, { bidirectional: false })
}
