...
const getTerminal = async () => {
  ...
  const { eventSource, error, close } = useEventSource(`/api/containers/${server}/${name}/terminal`)
  ...
  attachAddon.value = new AttachAddon(eventSource.value, { bidirectional: true, send: sendCommand, selector: 'data' })
}
