      ...
      <v-btn-group>
        <v-tooltip v-for="action in actions" :key="action.name" :text="action.name" location="bottom">
          <template #activator="{ props: props }">
            <v-btn v-bind="props" :color="action.color" class="my-1" :disabled="loading.includes('containers')"
        ...
        <v-dialog v-model="removeDialog">
          <v-card width="40vw" class="mx-auto">
            ...
            <v-card-text>
              Are you sure you want to permanently remove the following containers? <br />
              <pre class="text-error text-capitalize"><b>{{ selectedItems.map((item) => item).join(',\n') }}</b></pre>
...
const { refresh: refreshList,  } = useAsyncData('containerList', () => containersStore.fetchContainers())
const { refresh: refreshStats } = useAsyncData('containerStats', () => containersStore.fetchContainerStats())
...
const handleBulkAction = async (server: string, items: string[], action: string) => {
  if (action === "remove" && removeDialog.value === false) {
    ...
    const actionPromises = items.map((item) => containersStore.fetchContainerAction(server, item, action))
