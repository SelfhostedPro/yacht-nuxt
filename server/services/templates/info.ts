...
export const getTemplates = async (): Promise<YachtTemplate[]> => {
    ...
    const templateList = await configStorage.getKeys(config.paths.templates)
...
export const addTemplate = async ({ url, name: _name, title: _title }: addYachtTemplate) => {
    ...
    const exists = await configStorage.getItem(config.paths.templates + _name)
    if (!exists) {
        ...
        await configStorage.setItem(`${config.paths.templates}/${_name}.json`, template)
        return template
    } else {
